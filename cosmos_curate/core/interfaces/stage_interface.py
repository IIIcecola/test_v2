...
class PipelineTask:
    ...
    所有任务必须继承 PipelineTask；是在管道各阶段间传递的原子单位
    """
...
class CuratorStageSpec(StageSpec[PipelineTask, PipelineTask]):
    ...
    为流水线阶段添加额外配置（如并行度、重试策略等）
    Commonly used properties are:
    num_workers_per_node: set a fixed number of workers per node, typically for IO workers. 控制节点级并行度
    num_run_attempts_python: set the number of retry attempts in case of exceptions. 失败重试次数
    over_provision_factor: set the over-provision factor for the stage to influence stage worker allocation.
    worker_max_lifetime_m: set the maximum lifetime of a worker in minutes. worker最大生命周期（分钟）
