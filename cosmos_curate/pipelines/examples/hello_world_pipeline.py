...
class _LowerCaseStage(CuratorStage):
    ...
    @property
    def resources(self) -> CuratorStageResource: # ​​资源声明​​：需要1个CPU核心，无需GPU
...
class _GPT2Stage(CuratorStage):
    def __init__(self) -> None:
        self._model = GPT2() # 初始化模型

    @property
    def resources(self) -> CuratorStageResource:
        return CuratorStageResource(cpus=1.0, gpus=0.8, entire_gpu=False) # entire_gpu=False：允许与其他任务共享GPU
    ...
    def stage_setup(self) -> None:
        self._model.setup()  # 模型加载等预处理

    def process_data(self, tasks: list[HelloWorldTask]) -> list[HelloWorldTask] | None:  # type: ignore[override]
        for task in tasks:
            logger.debug(_get_stage_processing_log_str(self, task))
            # generate text based on the prompt and print out result 
            task.output = self._model.generate(task.prompt) # 文本生成
            print(" ".join(task.output.split()))  # noqa: T201，格式化输出。" ".join(...)：用单个空格重新连接拆分后的单词列表
            time.sleep(1)
        return tasks
