...
def _prepare_to_run_pipeline(
    ...
    """
    model_names: set[str] = set() # 创建空集合,集合（Set）是​​无序不重复​​元素的容器
...
T = TypeVar("T", bound=PipelineTask) # 定义了一个名为 T的类型变量，这个类型必须是 PipelineTask类或其子类。


def run_pipeline(
    input_tasks: list[T], # 列表中的每个元素都必须是 PipelineTask或其子类的实例。
    stages: list[CuratorStage | CuratorStageSpec], # 管道阶段配置列表，必须是CuratorStage | CuratorStageSpec的实例
    model_weights_prefix: str = MODEL_WEIGHTS_PREFIX, # 模型权重存储路径（默认为全局常量）
) -> list[T]: # 返回的列表中的元素类型也是 T
    ...
    # # 将混合的CuratorStage和CuratorStageSpec统一转换为标准规格
    stage_specs = _build_pipeline_stage_specs(stages) 

    # Run a few steps to prepare the pipeline for execution.
    # 下载或加载model_weights_prefix指定的模型
    try:
    ...
    pipeline_config = PipelineConfig(
        execution_mode=execution_mode, # 执行模式（STREAMING/BATCH）
        enable_work_stealing=False, # 是否允许空闲worker"偷取"其他worker的任务（负载均衡）
        return_last_stage_outputs=True, # 是否只返回最终阶段结果（设为True可节省内存）

        # Actor日志级别（始终关闭）
        actor_pool_verbosity_level=VerbosityLevel.NONE, # 云端环境关闭详细监控（VerbosityLevel.NONE）

        # 监控日志级别（条件设置）
        '''云端环境（非SLURM）→ 关闭监控日志（NONE）; 其他环境（本地/SLURM）→ 开启基础日志（INFO）'''
        ...
        mode_specific=StreamingSpecificSpec(
            max_queued_multiplier=1.5, # 队列最大容量为worker数的1.5倍
            max_queued_lower_bound=16, # 队列最小保证容量
            autoscaler_verbosity_level=VerbosityLevel.NONE, # 自动缩放日志
            executor_verbosity_level=VerbosityLevel.NONE, # 执行器日志
        ),
    )
    ...
    try:
        ...
        output_tasks = xenna_run_pipeline(pipeline_spec) # Ray分布式执行
    except Exception as e:
        # 异常捕获：记录并封装错误
        ...
        # 正常执行：处理结果
        if output_tasks is None: # 处理可能的None返回值
            logger.warning("Pipeline execution returned None")
            return []
        return output_tasks
    finally: # 无论成功与否都会执行finally块
        # 最终清理：关闭Ray集群
        ray_shutdown_delay = 5
        logger.info(f"Disconnecting from Ray cluster in {ray_shutdown_delay} seconds")
        ray_cluster_utils.shutdown_cluster(flush_seconds=ray_shutdown_delay) # 等待5秒确保所有任务完成
